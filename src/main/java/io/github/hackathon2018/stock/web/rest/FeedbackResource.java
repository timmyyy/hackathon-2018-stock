...
public class FeedbackResource {
    ...
    @Timed
    public List<Feedback> getAllFeedbacks(@RequestParam(required = false) String filter) {
        if ("request-is-null".equals(filter)) {
            log.debug("REST request to get all Feedbacks where request is null");
            return StreamSupport
                .stream(feedbackRepository.findAll().spliterator(), false)
                .filter(feedback -> feedback.getRequest() == null)
